...
describe("NeopolisTwo", function () {
  ...
  beforeEach(async function () {
    contract = await ethers.getContractFactory("NeopolisTwo");
  ...
  describe("constructor", function () {
    ...
    it("should initialize a starting tower", async function () {
      const initialTowerHeight = await neopolis.initialTowerHeight();
      const initialXPosition = await neopolis.initialXPosition();
      const initialYPosition = await neopolis.initialYPosition();

      expect(
        await neopolis.heightOf(initialXPosition, initialYPosition)
      ).to.equal(initialTowerHeight);
  ...
  describe("place", function () {
    it("should add to the height a tower if the tower doesn't topple", async function () {
      const initialXPosition = 2;
      const initialYPosition = 1;
      const initialTowerHeight = await neopolis.initialTowerHeight();

      await neopolis.place(initialXPosition, initialYPosition);

      const ownerBalance = await neopolis.balanceOf(owner.address);
      const towerHeight = await neopolis.heightOf(
        initialXPosition,
        initialYPosition
      );
    ...
    it("should transfer tokens equal to the height of a tower to the placer if the tower topples", async function () {
      ...
      await neopolis.connect(addr1).place(initialXPosition, initialYPosition);
